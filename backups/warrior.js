...
setInterval(function () {
  ...
  if (
    character.max_hp - character.hp > 300 ||
...
if (till_level === 0)
  function updateGUI() {
    let $ = p.$;
    let xp_percent = (
      (character.xp / p.G.levels[character.level]) *
      100
    ).toFixed(2);
    ...
    if (last_target) {
      ...
      let monsters_left = Math.ceil(
        xp_missing / (monster_xp * party_modifier * character.xpm)
      );
  ...
  function updateGUI() {
    let $ = p.$;
    let xp_percent = ((character.xp / G.levels[character.level]) * 100).toFixed(
      2
    );
