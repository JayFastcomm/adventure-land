declare global {
  interface Window {
    close_merchant();
    distance(
      from: IPosition | PositionReal,
      to: IPosition | PositionReal
    ): number;
   ...
   * @returns TRUE if you can move there, FALSE otherwise
   */
  function can_move_to(destination: {
    real_x: number;
    real_y: number;
  }): boolean;
  /**
  ...
  function smart_move(
    destination: IPosition | MapName | MonsterName,
    callback?: () => void
  );
  ...
  function is_on_cooldown(name: SkillName): Promise<boolean>;
  function use_hp_or_mp(): void;
  function is_moving(entity: Entity): Promise<boolean>;
  // function get_nearest_monster(args: {
  //   min_xp: number;
  //   max_att: number;
  // }): Entity;
  function change_target(target: Entity): void;
  function is_in_range(target: Entity, skill: string): Promise<boolean>;
  function get_player(name: string): Entity;
  function get_target_of(target: Entity): Entity;
  function show_json(e: any): void;
  function get_nearest_monster(targetAttributes: {
    max_att?: number;
    min_xp?: number;
    target?: Entity;
    no_target?: boolean;
